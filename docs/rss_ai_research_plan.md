...
| **邏輯** | 呼叫 Google TTS Long Audio Synthesis → 輸出 LINEAR16 WAV 到 GCS。idempotent ID = `episode_{script_id}` |
| **模型 / 聲線** | `cmn-TW-Wavenet-B` (`PODCAST_TTS_VOICE`)；`PODCAST_TTS_LANGUAGE_CODE=cmn-TW`；`PODCAST_TTS_LOCATION=global` |
| **timeout** | 1800 秒 (`PODCAST_TTS_TIMEOUT_SECONDS`) |
| **輸出** | GCS `gs://signal-brief-audio-…/podcasts/<date>/<script_id>.wav` + Firestore `rss_podcast_episodes` |
| **idempotency** | sub-bucket `<run_bucket>_audio`；既有 episode 且有 audio_url 且 `force=False` → 跳過 |

#### 成本

Google TTS Long Audio Wavenet：$0.016 / 1k chars（推估）。7500 字 ≈ $0.12/篇 × 1/天 = $3.6/月。
...
| 名 | 值 |
|---|---|
| `PODCAST_TTS_VOICE` | `cmn-TW-Wavenet-B` |
...
| Podcast TTS | google | `Long Audio Synthesis (cmn-TW-Wavenet-B / LINEAR16)` | — | — | cmn-TW |
