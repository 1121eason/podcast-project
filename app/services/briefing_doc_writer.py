...
from app.models.signal import BriefingSection
...
def _format_briefing_text(
    ...
    if signal_pool_health.get("high_importance_count"):
        lines.append(f"高重要度（≥70）：{signal_pool_health.get('high_importance_count')}")
...
def write_briefing_to_doc(
    ...
    text = _format_briefing_text(briefing_date, overview, sections, signal_pool_health)
